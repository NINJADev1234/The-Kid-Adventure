...
def on_overlap_tile(sprite, location):
    Kid.say_text("Whoa! How much lava is there", 2000, False)
    Kid.say_text("Dammit", 1000, False)
...
def on_overlap_tile2(sprite2, location2):
    global Kid
    animation.run_image_animation(Kid, assets.animation("""
        Died
    """), 150, False)
    music.knock.play()
...
def on_overlap_tile4(sprite4, location4):
    global Credits
    Kid.say_text("Wait", 2000, False)
    Kid.say_text("Isn't it...", 1000, False)
    Kid.destroy(effects.spray, 1000)
    scene.set_background_color(15)
    music.play_melody("B A G A G F A C5 ", 120)
    Credits = textsprite.create("CREDITS")
    Credits.set_position(77, 59)
    pause(1000)
    Credits.set_text("COOLBRO : CODER, ART OF KID, ART OF SPIKES, SOUNDS MAKER")
    pause(1000)
    Credits.set_text("SPECIAL THANKS TO : MICROSOFT(GAME ENGINE), MOM,DAD(FOR CHEERING ME)")
    pause(1000)
    Credits.set_text("GAME WAS WRITTEN ON JAVASCRIPT")
    pause(1000)
...
def on_overlap_tile6(sprite6, location6):
    ...
    game.show_long_text("NICE WORK!", DialogLayout.CENTER)
    game.show_long_text("NEW LEVEL UNLOCKED", DialogLayout.CENTER)
...
def on_overlap_tile8(sprite8, location8):
    Kid.say_text("Wait!", 1000, False)
    Kid.say_text("I don't think we should touch this", 1000, False)
scene.on_overlap_tile(SpriteKind.player,
    assets.tile("""
        myTile2
    """),
